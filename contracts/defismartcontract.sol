...
//  function strConcat( string memory _resellp, string memory _eventid, string memory _d, string memory _e) public  pure returns (string memory){
    // address add=msg.sender;
    // // Strings.toHexString(uint256(uint160(msg.sender)), 20);
    // bytes memory _ba = bytes(_add);
    // bytes memory _bb = bytes(_resellp);
    // bytes memory _bc = bytes(_eventid);
